...
export const ShowtimeSelectCard = ({
  ...
  return (
    <div
      className={`flex border p-1 rounded flex-col items-start gap-2 ${
        selected ? 'shadow-lg border-primary shadow-black/30' : ''
      }`}
    >
      <div className="flex justify-between w-full px-2">
        <div className="text-sm ">{showtime.Screen.projectionType}</div>
        <div className="text-sm ">{showtime.Screen.soundSystemType}</div>
      </div>
      <div className="flex justify-between w-full px-2">
        <div className="text-sm font-bold">
          {format(new Date(showtime.startTime), 'p')}
        </div>
        <div className="text-sm">Price: Rs.{showtime.Screen.price}</div>
      </div>
...
export const ShowRemainingSeats = ({ showtimeId }: { showtimeId: number }) => {
  const { data } = trpcClient.showtimes.seatsInfo.useQuery({ showtimeId })
